...
export interface RegularDocumentRegions {
...
export function getDocumentRegions(document: TextDocument): RegularDocumentRegions {
...
function scanTemplateRegion(scanner: Scanner, text: string): EmbeddedRegion | null {
  ...
  while (unClosedTemplate !== 0) {
    ...
    if (languageId !== 'regular-html' && start !== 0) {
